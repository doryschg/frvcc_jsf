...
@ManagedBean(name = "formulario110AnexoF610Controller")
@ViewScoped
public class Formulario110AnexoF610Controller implements Serializable {
	...
	private LocalDate fechaFacturaHasta=LocalDate.now();
	...
	public void cargarFormularios(int mesPeriodo, int gestionPeriodo) {
		...
		filtros.adicionar("tipoFormularioId", OperadorCriteria.IGUAL, ParametrosFRVCC.TIPO_FORMULARIO_110_ANEXO_F610);
	...
	public void cargarFormularios2() {
		setVerDetalle(false);
		vListaFormularios = new ArrayList<>();
		FiltroCriteria filtros = new FiltroCriteria();
	...
	public void adicionarFormulario() {
		...
		filtros.adicionar("tipoFormularioId", OperadorCriteria.IGUAL, ParametrosFRVCC.TIPO_FORMULARIO_110_ANEXO_F610);
		filtros.adicionar("nitCi", OperadorCriteria.IGUAL, contextoModel.getNitCi());
		filtros.adicionar("nitEmpleador", OperadorCriteria.IGUAL, vNuevoFormulario.getNitEmpleador());
		filtros.adicionar("estadoId", OperadorCriteria.IGUAL, "AC");
		...
		if (lista.getFormulariosReponse().isEmpty()) {
			...
			vNuevoFormulario.setTipoFormularioId(ParametrosFRVCC.TIPO_FORMULARIO_110_ANEXO_F610);
			...
			String vmes=vNuevoFormulario.getMesPeriodo()<10?"0":"";
			vNuevoFormulario.setNombreFormulario(
					"F110_" + vNuevoFormulario.getAnioPeriodo() + "_"+vmes+ vNuevoFormulario.getMesPeriodo() 
							+  "_" + vNuevoFormulario.getNitEmpleador()+"_610");
			...
			this.nuevoFormulario();
			RequestContext.getCurrentInstance().execute("toastr.error('Ya cuenta con un formulario para el periodo y NIT del empleador introducidos', 'Error')");
		}
		cargarFormularios2();
	}
...
//			filtros.adicionar("fechaFactura", OperadorCriteria.MAYOR_IGUAL, fechaFacturaValidaDesde(fechaFacturaHasta).toString());
	...
	public LocalDate fechaFacturaValidaDesde(LocalDate pFechaHasta) {
		System.out.println("fecha valido desde................."+ pFechaHasta.plusDays(-120));
	...
	public void filtrarComprasPosibles()
	{
		...
		switch (formulario.getMesPeriodo()) {
		  case 3: case 12:
			  fechaFacturaHasta=LocalDate.of(formulario.getAnioPeriodo(), formulario.getMesPeriodo(), 31);
		    break;
		  case 6: case 9:
			  fechaFacturaHasta=LocalDate.of(formulario.getAnioPeriodo(), formulario.getMesPeriodo(), 30);
		    break;
		}
		vListaSeleccionables=vListaDisponibles.stream().filter(compra->compra.getFechaFactura().isBefore(fechaFacturaHasta)).filter(compra->compra.getFechaFactura().isAfter(fechaFacturaHasta.plusDays(-120))).collect(Collectors.toList());
		System.out.println(formulario.getFechaPresentacion().plusDays(-120));
		System.out.println(vListaSeleccionables.size());
	}
	...
	{
		System.err.println("vListaSeleccioandosSinGuardar "+vListaSeleccioandosSinGuardar.size());
		System.err.println("vListaDesseleccionadosSinGuardar "+vListaDesseleccionadosSinGuardar.size());
...
public void modificarCamposFormulario() {
		...
		if(fechaEmisionValida(pCompra.getFechaFactura())) {
			if(clientesRestController.getClienteRestFormularios().desmarcarComprasFormulario(formulario.getId(),
				...
				ResultadoGenericoDto<String> vRespuesta=clientesRestController.getClienteRestCompras().modificarCompra(compraEdicion);
					if(clientesRestController.getClienteRestFormularios().seleccionarComprasFormulario(formulario.getId(),
							vComprasIds).isOk()) {
						mensajesBean.addMensajes(vRespuesta);
						if(vRespuesta.isOk()) {
							actualizartotales();
	...
	public Boolean fechaEmisionValida(LocalDate fechaEmisionFac) {
		...
		LocalDate fechaHasta=LocalDate.of(formulario.getAnioPeriodo(), formulario.getMesPeriodo(), dias);
		LocalDate fechaValidaDesde= fechaFacturaValidaDesde(fechaHasta);
		return (fechaEmisionFac.isAfter(fechaValidaDesde)||fechaEmisionFac.isEqual(fechaValidaDesde))&&(fechaEmisionFac.isBefore(fechaHasta) || fechaEmisionFac.isEqual(fechaHasta));
	}
	...
	public LocalDate getFechaFacturaHasta() {
		return fechaFacturaHasta;
	}
	public void setFechaFacturaHasta(LocalDate fechaFacturaHasta) {
		this.fechaFacturaHasta = fechaFacturaHasta;
	}
